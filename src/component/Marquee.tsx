...
export default function Marquee() {
  return (
    <div
      className="relative w-full overflow-hidden py-6 border-y border-black/5 dark:border-white/5"
      aria-hidden="true"
    >
      {/* Fade edges */}
      <div className="absolute left-0 top-0 bottom-0 w-20 z-10 bg-gradient-to-r from-[#f8fafc] dark:from-[#1e1e1e] to-transparent pointer-events-none" />
      <div className="absolute right-0 top-0 bottom-0 w-20 z-10 bg-gradient-to-l from-[#f8fafc] dark:from-[#1e1e1e] to-transparent pointer-events-none" />
      ...
      <div className="marquee-track flex gap-10 items-center whitespace-nowrap">
        {ITEMS.map(({ icon: Icon, label, color }, i) => (
          ...
          >
            ...
            <span className="text-sm font-mono text-gray-600/70 dark:text-white/70 font-medium">
              {label}
            </span>
          </div>
        ))}
